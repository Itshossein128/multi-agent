...
export class DefaultMemoryWritePolicy implements MemoryWritePolicy {
  async shouldRemember(candidate: MemoryCandidate): Promise<MemoryWriteDecision> {
    ...
    if (/\b(?:password|passwd|api[_ -]?key|access[_ -]?token|client[_ -]?secret)\s+(?:is|equals)\s+\S+/i.test(content)) return { remember: false, reason: "secret" };
    if (/\b(password|passwd|api[_ -]?key|access[_ -]?token|refresh[_ -]?token|client[_ -]?secret|private[_ -]?key)\b\s*["']?\s*[:=]|-----BEGIN [\w ]*PRIVATE KEY-----|\bBearer\s+\S+|\b(?:sk|ghp|github_pat)[_-][a-zA-Z0-9_-]{12,}|\bAKIA[A-Z0-9]{16}\b/i.test(serialized)) return { remember: false, reason: "secret" };
    if (content.length < 12 || !/[\p{L}]/u.test(content) || /^(?:ok(?:ay)?|thanks?(?: you)?|hello|hi|done|success|test|acknowledged|sounds good|task completed)[\s!.]*$/i.test(content)) return { remember: false, reason: "trivial" };
