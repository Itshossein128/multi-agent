...
- **Agent execution policies are modeled but not enforced.** `filesystem`, `shell`, `network`, `workspaceRoot`, and `allowedCommands` exist in `AgentExecutionPolicy`, but no runtime executor applies them. Evidence: `packages/types/src/index.ts`, `src/agents/runtime/agentExecutorFactory.ts`, and `src/agents/runtime/notImplementedExecutor.ts`.
- **CLI and local backends are configuration-ready, not implemented.** API execution works; Codex, Claude Code, agy, Ollama, and LM Studio execution paths remain placeholders. Evidence: `src/agents/runtime/notImplementedExecutor.ts`.
