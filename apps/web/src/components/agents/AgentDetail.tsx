...
const sections = ["Overview", "Configuration", "Backend", "Tools", "Memory", "Workflows", "Executions"] as const;
...
export function AgentDetail({ agentId }: { agentId: string }) {
  ...
  const update = (patch: Partial<AgentRecord>) => { if (draft) setDraft({ ...draft, ...patch }); setNotice(""); };
  const beginEdit = () => {
  ...
  return <main className="min-h-screen bg-zinc-950 text-zinc-100">
    <div className="mx-auto max-w-7xl space-y-5 p-4 lg:p-8">
      ...
      {agent && <>
        ...
        {section === "Memory" && <Section title="Memory"><p className="text-sm text-zinc-300">No agent-level memory configuration is represented in the current model.</p><p className="text-sm text-zinc-400">Memory type, scope and read/write mode are configured on workflow memory nodes. Those nodes belong to the workflow and are not automatically assigned to this agent. Inspect them in the Graph Editor.</p><Button variant="outline" onClick={() => setSection("Workflows")}>View workflow usage</Button></Section>}
