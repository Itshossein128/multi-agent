  ...
  return <div className="space-y-3">
    ...
    <ol className="max-h-96 space-y-2 overflow-auto" aria-label="Execution timeline">
      {events.map((event) => <li key={event.id}>
        <button type="button" aria-pressed={event.id === selectedId} onClick={() => setSelectedId(event.id)}
          ...
          <time className="mt-1 block text-zinc-400" dateTime={event.timestamp}>{event.timestamp}</time>
    ...
    {selected && <section className="rounded-lg border border-zinc-700 p-3" aria-label="Event detail">
      ...
      <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 break-all text-xs text-zinc-300">
        {Object.entries({ Timestamp: selected.timestamp, Run: selected.runId, Node: selected.nodeId, Agent: selected.agentId, Tool: selected.toolId,
          "Duration (ms)": selected.payload.durationMs }).map(([label, value]) => value !== undefined && <div key={label} className="contents"><dt>{label}</dt><dd>{String(value)}</dd></div>)}
      </dl>
