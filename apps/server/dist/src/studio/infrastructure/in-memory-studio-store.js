...
class InMemoryStudioStore {
    ...
    async getAgent(id, principal) {
        ...
        if (principal) {
            ...
            if (!agent.tenantId || agent.tenantId !== principal.tenantId)
                return null;
            if (agent.ownerId && agent.ownerId !== principal.userId)
                return null;
        }
        return structuredClone(agent);
    }
    ...
    async getTool(id, principal) {
        ...
        if (principal) {
            ...
            if (!tool.tenantId || tool.tenantId !== principal.tenantId)
                return null;
            if (tool.ownerId && tool.ownerId !== principal.userId)
                return null;
        }
        return structuredClone(tool);
    }
    ...
    async deleteTool(id, principal) {
        if (principal) {
            ...
            if (existing) {
                if (existing.isSystem)
                    throw new Error("Cannot delete system tool");
                if (!existing.tenantId || existing.tenantId !== principal.tenantId || (existing.ownerId && existing.ownerId !== principal.userId)) {
                    throw new Error("Access denied to tool");
                }
            }
