...
function getPeriod(isoOrTimestamp) {
    ...
    app.get("/", async (c) => {
        ...
        const agents = baseAgents.map((agent) => {
            ...
            return {
                ...
                role: agent.role,
                status,
                currentTask: activeTask?.title || activeRun?.metadata?.name || (isRunning ? "Executing active workflow" : undefined),
        ...
        for (const task of tasks) {
            if (task.status === "todo" || task.status === "planning") {
                ...
                queue.push({
                    ...
                    queuedAt: task.createdAt || (0, types_1.nowIso)(),
        ...
        for (const run of runs) {
            if (run.status === "queued" && (!run.taskId || !queuedTaskIds.has(run.taskId))) {
                queue.push({
                    id: run.id,
                    title: run.metadata?.name || run.metadata?.title || `Workflow Run ${run.id.slice(0, 8)}`,
                    agentRole: run.metadata?.agentName || "Orchestrator Agent",
                    priority: "high",
        ...
        for (const run of runs) {
            if (run.status === "completed") {
                if (run.taskId)
                    completedTaskIds.add(run.taskId);
                ...
                completedTasks.push({
                    id: run.id,
                    title: meta.name || meta.title || `Workflow Run ${run.id.slice(0, 8)}`,
                    agent: meta.agentName || "Orchestrator Agent",
