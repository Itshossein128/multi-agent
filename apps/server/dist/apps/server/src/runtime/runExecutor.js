...
class RunExecutor {
    ...
    constructor(store = new runStore_1.InMemoryRunStore(), agentRuntime = new runtime_1.AgentRuntime(), checkpointer, telemetry = telemetry_1.ExecutionTelemetry.disabled(), guardrails = (0, guardrails_1.runtimeGuardrailsFromEnvironment)()) {
        this.store = store;
    ...
    cancel(runId) {
        ...
        if (entry.run.status === "waiting_for_human") {
            ...
            this.store.append(runId, { id: (0, types_1.uid)("event"), runId, type: "run.failed", timestamp: (0, types_1.nowIso)(), sequence: 0, payload: { cancelled: true } });
            return true;
        }
    ...
    fail(runId, error) {
        ...
        this.store.update(runId, { status: this.store.signal(runId)?.aborted ? "cancelled" : "failed", completedAt: (0, types_1.nowIso)(), error: message });
        this.store.append(runId, { id: (0, types_1.uid)("event"), runId, type: "run.failed", timestamp: (0, types_1.nowIso)(), sequence: 0, payload: { error: message } });
        logging_1.log.error("run.failed", { runId, workflowId: this.store.get(runId)?.run.workflowId, taskId: this.store.get(runId)?.run.taskId, error: message });
    }
}
