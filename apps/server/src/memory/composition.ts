...
export function createMemoryComposition(): MemoryComposition {
  ...
  if (mode === "disabled") return { close: async () => {} };
  ...
  const service = new DefaultMemoryService(store, { embeddingProvider: embeddingProviderFromEnvironment(), defaultTtlMs: ttlDays === 0 ? undefined : ttlDays * 86400000 });
