...
export function validateWorkflow(definition: WorkflowDefinition, agents: AgentRecord[], limits: WorkflowValidationLimits = {}): WorkflowIssue[] {
  ...
  for (const node of definition.nodes) {
    if (!node.id?.trim()) add("error", "INVALID_NODE_ID", "Node id is required.", node.id);
  ...
  for (const edge of definition.edges) {
    if (!edge.id?.trim()) add("error", "INVALID_EDGE_ID", "Edge id is required.", undefined, edge.id);
    if (edgeIds.has(edge.id)) add("error", "DUPLICATE_EDGE_ID", `Duplicate edge id "${edge.id}"`, undefined, edge.id);
    edgeIds.add(edge.id);
    if (!nodes.has(edge.source)) add("error", "INVALID_EDGE_REFERENCE", `Edge references missing source node "${edge.source}"`, undefined, edge.id);
    if (!nodes.has(edge.target)) add("error", "INVALID_EDGE_REFERENCE", `Edge references missing target node "${edge.target}"`, undefined, edge.id);
    if (edge.source === edge.target) add("error", "UNSAFE_CYCLE", "Self-referential edges are not supported.", edge.source, edge.id);
    if (edge.kind === "conditional" && !edge.branchKey.trim()) add("error", "MISSING_BRANCH_KEY", "Conditional edge is missing a branch key", undefined, edge.id);
    if (edge.kind !== "conditional" && edge.branchKey.trim()) add("warning", "UNUSED_BRANCH_KEY", "Normal edges ignore branch keys.", undefined, edge.id);
    const source = nodes.get(edge.source), target = nodes.get(edge.target);
    if (target?.type === "input") add("error", "INVALID_CONNECTION", "Input nodes cannot have incoming edges.", target.id, edge.id);
    if (source?.type === "output") add("error", "INVALID_CONNECTION", "Output nodes cannot have outgoing edges.", source.id, edge.id);
    if (source?.type === "condition" && edge.kind !== "conditional") add("error", "AMBIGUOUS_BRANCHING", "Condition nodes require conditional outgoing edges.", source.id, edge.id);
    if (edge.kind === "conditional" && source?.type !== "condition" && source?.type !== "approval") add("error", "INVALID_CONDITIONAL_SOURCE", "Conditional edges may only leave condition or approval nodes.", source?.id, edge.id);
  }
  ...
  for (const node of definition.nodes) {
    ...
    if (node.type === "approval") {
      ...
      if (!config.message?.trim()) add("error", "MISSING_APPROVAL_MESSAGE", "Approval node requires a message.", node.id);
    ...
    if (node.type === "condition") {
      ...
      if (branches.some(branch => !branch.key?.trim())) add("error", "INVALID_BRANCH_KEY", "Condition node has an empty branch key.", node.id);
...
function detectCycles(definition: WorkflowDefinition, nodes: Map<string, WorkflowNode>, add: (level: WorkflowIssue["level"], code: string, message: string, nodeId?: string, edgeId?: string) => void) {
  ...
  const walk = (id: string) => {
    if (visiting.has(id)) { add("error", "UNSAFE_CYCLE", "Cycles are not supported until explicit loop semantics are implemented.", id); return; }
